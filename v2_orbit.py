...
conn = krpc.connect(name='Put V1 into Orbit')
...
liq_fuel = conn.add_stream(stage_2_resources.amount, 'LiquidFuel')
...
while True:
    ...
    if not liq_separated:
        if liq_fuel() < 0.1:
            vessel.control.activate_next_stage()
            liq_separated = True
            print('Liquid rocket separated')
